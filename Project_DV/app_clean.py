...
app.layout = html.Div([
                    ...
                    ], className='pretty'), #info

                    html.Div([
                        html.Div([
                            html.Div([dcc.Graph(id='choropleth')], className='column60 pretty'),
                            html.Div([dcc.Graph(id='scatter_graph')], className='column40 pretty')
                    ...
                    html.Div([
                        ...
                        html.Div([
                            html.Div([dcc.Graph(id='line_graph')], className='column50 pretty'),
                        ], className='column50'), # linhas
                        html.Div([
...
def plots(lin_log, year, var, exp, country,top_low):
    ...
    data_line = go.Scatter(x=df_refugees_1['Year'].values, y=df_refugees_1[exp],mode='lines', name=str(exp))
    layout_line = go.Layout(title=str(exp) + ' over years for '+country,  xaxis=dict(title='Year',showgrid=True), yaxis=dict(title=str(exp),showgrid=True))
    line_graph = go.Figure(data=data_line, layout=layout_line)
    line_graph.add_trace(go.Scatter(x=df_refugees_1['Year'].values, y=df_refugees_1[var], mode='lines', yaxis="y2", name=str(var)))
    line_graph.update_layout(yaxis2=dict(title=str(var), side="right", overlaying="y"), legend=dict(x=0.05, y=-.3), legend_orientation="h")
